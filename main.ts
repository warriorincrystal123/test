...
basic.forever(function () {
    while (pins.digitalReadPin(DigitalPin.P1) == 1 && (timeanddate.time(timeanddate.TimeFormat.HHMMSS24hr) >= "09:00.00" && timeanddate.time(timeanddate.TimeFormat.HHMMSS24hr) <= "09:05.00")) {
        music.playMelody("C C5 C C5 C C5 C C5 ", 250)
    }
})
basic.forever(function () {
    if (pins.digitalReadPin(DigitalPin.P1) == 0 && (timeanddate.time(timeanddate.TimeFormat.HHMMSS24hr) >= "09:00.00" && timeanddate.time(timeanddate.TimeFormat.HHMMSS24hr) <= "09:05.00")) {
        basic.showIcon(IconNames.Yes)
        music.stopAllSounds()
    }
    if (pins.digitalReadPin(DigitalPin.P1) == 1 && timeanddate.time(timeanddate.TimeFormat.HMMSSAMPM) == "09:05.00") {
